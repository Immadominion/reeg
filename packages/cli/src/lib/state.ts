import { mkdirSync, readFileSync, writeFileSync } from 'node:fs';
import { homedir } from 'node:os';
import { join } from 'node:path';

// Local, per-operator state: which working directory belongs to which environment. This is a
// convenience cache only; the authoritative record is on chain, so losing it loses nothing that
// cannot be re-derived (the workdir is rebuilt by `reeg restore`).
export interface MachineState {
  workdir: string;
  network: string;
  createdAt: string;
  /** The Machine's shared AccessPolicy id, present for shareable Machines. */
  policyId?: string;
}

interface StateFile {
  machines: Record<string, MachineState>;
}

function root(): string {
  return process.env.REEG_HOME ?? join(homedir(), '.reeg');
}

function statePath(): string {
  return join(root(), 'state.json');
}

function read(): StateFile {
  try {
    return JSON.parse(readFileSync(statePath(), 'utf8')) as StateFile;
  } catch {
    return { machines: {} };
  }
}

function write(state: StateFile): void {
  mkdirSync(root(), { recursive: true });
  writeFileSync(statePath(), `${JSON.stringify(state, null, 2)}\n`);
}

/** The working directory for a Machine, created on demand. */
export function workdirFor(machineId: string): string {
  return join(root(), 'machines', machineId, 'work');
}

/** The persisted command log path for a Machine. */
export function logFor(machineId: string): string {
  return join(root(), 'machines', machineId, 'log.json');
}

/** The agent memory directory for a Machine, a sibling of the working directory. A command's
 *  memory backend writes here (via REEG_MEMORY_DIR); checkpoint captures it into the bundle. */
export function memoryFor(machineId: string): string {
  return join(root(), 'machines', machineId, 'memory');
}

export function recordMachine(
  machineId: string,
  network: string,
  createdAt: string,
  policyId?: string,
): MachineState {
  const state = read();
  const entry: MachineState = { workdir: workdirFor(machineId), network, createdAt, policyId };
  state.machines[machineId] = entry;
  mkdirSync(entry.workdir, { recursive: true });
  mkdirSync(memoryFor(machineId), { recursive: true });
  write(state);
  return entry;
}

export function getMachine(machineId: string): MachineState {
  const entry = getMachineOrNull(machineId);
  if (!entry) {
    throw new Error(`unknown environment ${machineId}; create it with 'reeg create' first`);
  }
  return entry;
}

/** Like getMachine, but returns null instead of throwing. Used by restore, which a grantee can
 *  run for a Machine they never created locally. */
export function getMachineOrNull(machineId: string): MachineState | null {
  return read().machines[machineId] ?? null;
}
