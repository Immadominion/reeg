 ...
 * base64(flag || 32-byte secret); flag 0 is Ed25519. Nothing here logs the key, and the server
 * never returns key material to the agent — only signatures and resulting object ids/digests.
 * (This is today's path; the delegated/sponsored signer of docs/03-engineering/agent-access.md
 * swaps in here later without changing any tool surface.)
 */
...
/** A bare Sui client, all a read/verify tool needs. */
export function suiClient(config: ServerConfig): SuiJsonRpcClient {
  return new SuiJsonRpcClient({ url: config.rpcUrl, network: config.network });
}

/** Construct the Sui, Walrus, and Seal clients from config (for checkpoint/restore). Walrus and
 *  Seal are only meaningful on testnet/mainnet; the network is narrowed accordingly. */
export function buildClients(config: ServerConfig): Clients {
